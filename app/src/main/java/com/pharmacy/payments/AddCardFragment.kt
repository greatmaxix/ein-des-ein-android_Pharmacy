...
import com.pharmacy.core.extensions.isLetterAndSpace
import com.pharmacy.core.extensions.mockToast
import com.pharmacy.core.extensions.toast
import com.pharmacy.util.CreditCardExpiryInputFilter
import com.pharmacy.util.CreditCardInputFilter
...
class AddCardFragment : PaymentsBaseFragment(R.layout.fragment_add_card) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(tilCardNumber) {
            editText?.filters = arrayOf(LengthFilter(com.pharmacy.payments.AddCardFragment.CREDIT_CARD_NUMBER_LENGTH), CreditCardInputFilter())
            updateStateAfterTextChanged {
                it.length == com.pharmacy.payments.AddCardFragment.CREDIT_CARD_NUMBER_LENGTH
            }
            setEndIconOnClickListener {
                if (tilCardNumber.editText?.text?.length != com.pharmacy.payments.AddCardFragment.CREDIT_CARD_NUMBER_LENGTH) tilCardNumber.editText?.text = null
            }
        }
        with(tilCardExpDate) {
            editText?.filters = arrayOf<InputFilter>(CreditCardExpiryInputFilter())
            updateStateAfterTextChanged {
                it.length == com.pharmacy.payments.AddCardFragment.CREDIT_CARD_EXP_DATE_LENGTH
            }
            setEndIconOnClickListener {
                if (tilCardExpDate.tag == null) {
                    requireContext().toast("TODO show info") // TODO what to show
                } else if (tilCardExpDate.tag as Boolean) {
                    if (tilCardExpDate.editText?.text?.length != com.pharmacy.payments.AddCardFragment.CREDIT_CARD_EXP_DATE_LENGTH) tilCardExpDate.editText?.text = null
                }
            }
        }
        with(tilCardCVV) {
            updateStateAfterTextChanged {
                it.length == com.pharmacy.payments.AddCardFragment.CREDIT_CARD_CVV_LENGTH
            }
            setEndIconOnClickListener {
                if (tilCardCVV.tag == null) {
                    requireContext().toast("TODO show info") // TODO what to show
                } else {
                    if (tilCardCVV.editText?.text?.length != com.pharmacy.payments.AddCardFragment.CREDIT_CARD_CVV_LENGTH) tilCardCVV.editText?.text = null
                }
            }
        }
