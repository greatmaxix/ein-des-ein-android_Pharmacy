...
class RegionRepository(private val rm: RestManager, private val customerDAO: CustomerDAO) : KoinComponent {
    ...
    suspend fun saveRegionLocally(region: Region) {
        regionDAO.insert(TemporaryRegion(region.id, region.name))
        rm.setLocalRegion(region.id)
    }

    suspend fun updateCustomerRegion(id: Int): ResponseWrapper<BaseDataResponse<CustomerInfoItem>> {
        rm.setLocalRegion(id)
